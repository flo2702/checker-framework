...
/**
 * The abstract annotated type factory for the freedom-before-commitment type system. When using the
 ...
 *
 * @see InitializationAnnotatedTypeFactory
 */
public abstract class InitializationAbstractAnnotatedTypeFactory<
     ...
     */
    public InitializationAbstractAnnotatedTypeFactory(BaseTypeChecker checker) {
        super(checker);
    }
     ...
     */
    protected abstract @Nullable InitializationFieldAccessAbstractAnnotatedTypeFactory<Value, Store, Transfer, Analysis>
                    getFieldAccessFactory();

    @Override
    protected Analysis createFlowAnalysis() {
    ...
    @Override
    public Store getExceptionalExitStore(Tree tree) {
    ...
    protected void setSelfTypeInInitializationCode(
        ...
        if (areAllFieldsInitializedOnly(enclosingClass)) {
            ...
            Store initStore = getStoreBefore(tree);
     ...
     * </ul>
     *
     * <p>See {@link #getUninitializedFields(InitializationAbstractStore, TreePath, boolean,
     * Collection)} for a method that does not require the target checker.
     *
