...
(Section~\refwithpage{build-source}), or download jars that continuous
integration built from it (Section~\refwithpage{development-jars}).
\end{itemize}
...
\item
  ...
  \\
  %END LATEX
  \url{https://eisop.github.io/cf/checker-framework-3.49.5-eisop1.zip}
...
If your project already uses \href{https://errorprone.info/}{Error Prone}, you can instead
run the Checker Framework as an Error Prone plugin, so that its type systems run as part of
your existing Error Prone compilation rather than as a separate annotation processor.  See
Section~\ref{error-prone}.

You can also run a checker from within your favorite IDE or build system.  See
Chapter~\ref{external-tools} for details about tools such as
Ant (Section~\ref{ant-task}),
Bazel (Section~\ref{bazel}),
Buck (Section~\ref{buck}),
Error Prone (Section~\ref{error-prone}),
...
the javac Diagnostics Wrapper for post-processing a checker's output
(Section~\ref{javac-diagnostics-wrapper});
...
\item \<-AassumeAssertions=enabled|disabled|neither>
  Whether to assume that assertions (Java \<assert> statements) are enabled
  or disabled.  The default, \<neither>, accounts for both cases; see
...
\item \<-AignoreDeadCode>
  Do not check dead (unreachable) code: literal-condition branches such as
  \code{if (false)}, and code that dataflow determines can never be reached
  (such as a catch block for an exception type the try block can never
  throw). By default, the Checker Framework checks dead code too, since it
  might become reachable after a future edit.
\end{itemize}
...
\item \<-AwarnBytecodeConflicts>
  Warn about contradictory declaration annotations, such as two
  \refqualclass{framework/qual}{DefaultQualifier} annotations that set the same
  location in one hierarchy to different qualifiers, on a declaration read from
  bytecode.  Such a declaration is in already-compiled code, and only those
  elements that the compilation happens to examine are checked, so this is off
  by default.
...
\item \<-AuseConservativeDefaultsForUncheckedCode>
  ...
  \refqualclass{framework/qual}{AnnotatedFor} annotation, ``bytecode'' specifies
...
\begin{itemize}
\item \<-Amode=\emph{mode}>
  Enable a checker-defined group of options.  Each checker documents the modes it supports and
  the options each one enables.  A mode only sets an option that you did not set yourself, so an
  option you write keeps the value you gave it; but most options are on/off flags with no
  negative form, so writing one cannot turn off what a mode enables.
\item \<-Alint>
...
\noindent
The annotations in \code{MyNullPermittingMultiset} and
\code{MyNullProhibitingMultiset} are on type arguments of \code{Multiset},
which is permitted.  By contrast, an annotation immediately before the class
or interface name in an \code{extends} or \code{implements} clause is a main annotation
on the supertype.  By default, the Checker Framework reports an
\code{annotation.on.supertype} error for such an annotation.

The Tainting Checker permits main annotations on supertypes because it
tracks taintedness when a value of a subclass type is used as a value of a
supertype.  For example, in
\code{@Untainted class UntaintedBuffer extends @Untainted Buffer},
the first \code{@Untainted} is a type-declaration bound of
\code{UntaintedBuffer}, and the second is the main annotation on
\code{Buffer}.  The Tainting Checker verifies that the former is a subtype of
the latter.
...
% LocalWords:  AinferenceWorkBudget
