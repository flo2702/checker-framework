...
The \<-AassumeAssertions>
command-line option (Section~\ref{type-refinement-assertions}) does not
...
information regardless of the \<-AassumeAssertions>
command-line option.
...
A few warnings are issued before any source file is processed, so they have no
source position:  there is no declaration on which a \<@SuppressWarnings>
annotation could be written, and \<-AsuppressWarnings> is the only way to
suppress them.  They warn that an annotation file is being text-parsed rather
than read from a binary stub file, which is slower
(Section~\ref{annotating-jdk}):
...
\item
  \<-AsuppressWarnings=text.parsing.jdk.class>:  a class of the annotated JDK is
  missing from the binary stub file that was read.
\item
  \<-AsuppressWarnings=text.parsing.stub>:  a stub file that a checker
  distributes has no binary stub file.
\item
  \<-AsuppressWarnings=text.parsing.command.line.stub>:  a \<-Astubs> location has
  an incomplete binary stub setup -- some, but not all, of its \<.astub> files
  were read from a pre-parsed binary form (Section~\ref{stub-using-binary}).
\item
  \<-AsuppressWarnings=stale.binary.stub>:  a \<-Astubs> file's binary stub does
  not match the file's current content, so the file was text-parsed instead
  (Section~\ref{stub-using-binary}).
\end{itemize}

\noindent
\<-AsuppressWarnings=text.parsing> suppresses the first four, but not
\<stale.binary.stub>, whose key does not share that prefix.  Rather than
suppressing these warnings, it is usually better to generate the missing or
stale binary stub file, as described in Sections~\ref{annotating-jdk},
\ref{stub-distributing}, and~\ref{stub-using-binary}.
...
The Nullness Checker treats JSpecify's \code{@NullMarked} as an alias for
\code{@AnnotatedFor}, and \code{@NullUnmarked} as an alias for \code{@UnannotatedFor},
both scoped to nullness checking with \code{applyToSubpackages = false},
so \code{-AonlyAnnotatedFor} gives the same behavior for \code{@NullMarked} code
(see Section~\ref{nullness-jspecifyNullMarkedAlias}).
A written \code{@AnnotatedFor} composes with an aliased one, so
\code{@AnnotatedFor("index") @NullMarked} is checked by both the Index Checker and the
Nullness Checker.
You can exclude an element from an enclosing \code{@AnnotatedFor} scope with
\refqualclass{framework/qual}{UnannotatedFor}; see Section~\ref{compiling-libraries}.
...
% LocalWords:  refactored AassumeAssertions assertNotNull
% LocalWords:  NullnessUtil checkNotNull
