...
These examples illustrate the general rules of
Section~\ref{generics-bounds-syntax} and Section~\ref{generics-defaults},
which are the authoritative specification.  Recall the two rules that those
sections state:  a primary annotation written directly on the type variable
(as in \<@Nullable T>) sets only the \emph{lower} bound; and if the
\<extends> clause is omitted, then the \emph{upper} bound defaults
independently to \<@\emph{TopType} Object>, regardless of the primary
annotation.
...
\begin{Verbatim}
  ...
  class MyList1a<@Nullable T extends @Nullable Object> { ... } // same as MyList1 (see below)
  ...
  class MyList4<@NonNull T> { ... }                            // same as MyList3, NOT MyList2 (see below)
\end{Verbatim}

\<MyList1> and \<MyList1a> mean the same thing, but only because
\<@Nullable> is the \emph{top} qualifier of the Nullness type hierarchy.
\<MyList1> omits the \<extends> clause, so its upper bound defaults to the
top qualifier \<@Nullable Object>; \<MyList1a> writes that same bound
explicitly.  Do not over-generalize this coincidence into a rule that a
primary annotation is copied onto the upper bound --- it is not.  This is
shown by \<MyList4>:  \<@NonNull> is not the top qualifier, so
\<MyList4>'s omitted \<extends> clause still defaults the upper bound to the
top qualifier \<@Nullable Object>.  The primary \<@NonNull> sets only the
lower bound (which for a type variable already defaults to the bottom
qualifier \<@NonNull>).  Therefore \<MyList4> means the same as \<MyList3>,
and is emphatically \emph{not} the same as \<MyList2>:  \<MyList4> may be
instantiated with a \<@Nullable> type, whereas \<MyList2> may not.  The Map
Key Checker relies on exactly this rule when it writes a bare lower-bound
annotation such as \<class Set<@KeyForBottom E>> to widen a type
parameter's lower bound without constraining its upper bound; see
Section~\ref{map-key-defaults-lowerbound}.
...
Type argument inference for a single method invocation can be expensive: it is
roughly cubic in the nesting depth of the invocation, so a deeply nested, often
machine-generated, invocation could make the checker run for many seconds.  To
bound this, the checker abandons the type argument inference of an invocation
once it exceeds a work budget, issuing a \code{type.argument.inference.budget}
error that asks you to supply explicit type arguments as shown above.  The
default budget has about an order of magnitude of headroom over the most complex
hand-written code, so it does not affect realistic programs.  If legitimate
(typically machine-generated) code triggers the error, raise the budget using
the \code{-AinferenceWorkBudget=}\emph{N} command-line option.
