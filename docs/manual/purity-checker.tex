...
\item[\refqualclass{dataflow/qual}{Deterministic}]
  ...
  arguments and in the same environment (the Java heap, including all
  fields of all objects and static variables), then it returns the
  identical result according to \<==> (not just according to \<equals()>).
  Determinism alone does not prevent a method from modifying the heap;
  repeated calls to a deterministic method may return different results if
  the heap changed in between (including changes made by the method itself).

\item[\refqualclass{dataflow/qual}{Pure}]
  indicates that the method is both \<@SideEffectFree> and \<@Deterministic>.
  To guarantee that repeated calls return the same value, a method must be
  \<@Pure>.
...
Furthermore, the Checker Framework currently assumes repeated calls to
any \<@Deterministic> method return the same value even if the method is not
...
\<@Deterministic> methods. Therefore, you should not write \<@Deterministic> on a
method that has side effects; write \<@Pure> instead.
