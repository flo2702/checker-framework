...
locations where the programmer did not write an annotation.  (However, if the
class or its package carries an applicable
\refqualclass{framework/qual}{AnnotatedFor} annotation in bytecode, the code is
treated as checked code and ordinary defaults apply instead; see
Section~\ref{AuseConservativeDefaultsForUncheckedCodesource}.)  Unless otherwise
noted, the choice is:
...
arguments and in the same environment.
...
the same arguments and in the same environment, use the
\refqualclass{dataflow/qual}{Deterministic} annotation.
Actually, it is necessary to use \refqualclass{dataflow/qual}{Pure} which
means both \<@Deterministic> and \<@SideEffectFree>, because determinism
alone does not prevent a method from modifying the heap (or having an
intervening method do so), which may cause subsequent calls to return a
different value.

(Currently, the Checker Framework optimistically assumes that repeated invocations
of any \<@Deterministic> method return the same value even if the method is not
\<@SideEffectFree>. This assumption is sound only when purity checking is enabled
via \<-AcheckPurityAnnotations>, because purity checking forbids side effects in
\<@Deterministic> methods. Therefore, do not write \<@Deterministic> on a method
that has side effects; write \<@Pure> instead.)
...
could happen at run time, whether assertions are enabled or disabled.  The
\<-AassumeAssertions> command-line option states what to assume instead.  It
takes one of three values:

\begin{itemize}
  \item
  \<-AassumeAssertions=enabled> assumes that assertions (that is, Java
  \<assert> statements) are enabled, as if Java is run with the \<-ea> or
  \<-enableassertions> command-line argument.
  \item
  \<-AassumeAssertions=disabled> assumes that assertions are disabled, as if
  Java is run with the \<-da> or \<-disableassertions> command-line argument.
  \item
  \<-AassumeAssertions=neither> makes neither assumption.  It accounts for
  both possibilities, warning about any error that could happen under either
  one.  This is the default, so supplying \<-AassumeAssertions=neither> is
  equivalent to supplying no \<-AassumeAssertions> option at all.
\end{itemize}

\noindent
It is uncommon to supply the option.

The \<-AassumeAssertionsAreEnabled> and \<-AassumeAssertionsAreDisabled>
command-line options are deprecated in favor of \<-AassumeAssertions=enabled>
and \<-AassumeAssertions=disabled>.  Each is still honored, but supplying one
issues a warning, and they will be removed in a future release.

This command-line option has no effect on processing of \<assert>
...
% LocalWords:  AassumeAssertions myArray vals propkey forName
...
% LocalWords:  wildcards guieffect readLine
